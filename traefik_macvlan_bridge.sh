...
[MACVLAN]
Mode=bridge" | sudo tee "$HOST_MACVLAN_SYSTEMD_FILE" > /dev/null
...
configure_prereboot_systemd_network() {
    ...
    # That interface is fully systemd-networkd now. So the ifupdown2 will no longer be getting an IP address.    
    echo "[Match]
...
MulticastDNS=false
LLMNR=false" | sudo tee "$HOST_INTERFACE_SYSTEMD_FILE" > /dev/null


    # Create the macvlan interface network file
    HOST_MACVLAN_INTERFACE_SYSTEMD_FILE="/etc/systemd/network/25-traefik2docker.network"

    # You will need to give the new bridge an IP address.
    echo "[Match]
Name=traefik2docker

# Address in the same subnet as the Traefik MacVLAN
[Network]
Address=$HOST_IP_SUBNET
Gateway=$HOST_GATEWAY" | sudo tee "$HOST_MACVLAN_INTERFACE_SYSTEMD_FILE" > /dev/null
...
HOST_MACVLAN_INTERFACE_SYSTEMD_FILE=${HOST_MACVLAN_INTERFACE_SYSTEMD_FILE}
...
GF_AUTH_GENERIC_OAUTH_CLIENT_ID=KNAvYh4xTxyjhsHceBMLUfHNCW4UGsOLktBDBjmT
GF_AUTH_GENERIC_OAUTH_CLIENT_SECRET=jK7ioYJDltMBTIVSHm150Y4pQxf4DbNp28bAo2X6sqiyfAk1XSbXwzbbuKsA5esX1ZTtt0vZqM7VwRgB3topj5dnx7kFw7v915TZH8ttrdGBUmM9KoAM9LaDHwINKu8e
GF_AUTH_GENERIC_OAUTH_AUTH_URL=http://auth.${DOMAIN}/application/o/authorize/
GF_AUTH_GENERIC_OAUTH_TOKEN_URL=http://auth.${DOMAIN}/application/o/token/
GF_AUTH_GENERIC_OAUTH_API_URL=http://auth.${DOMAIN}/application/o/userinfo/
...
# You must generate a database password and Authentik secret key.
    dpkg -l | grep -qw openssl || sudo apt install -y openssl 
...
setup_docker_networks() {
    # Create proxy2traefik if it doesn't exist
    if ! docker network inspect proxy2traefik >/dev/null 2>&1; then
        echo "      # Creating internal docker network: proxy2traefik"
        docker network create proxy2traefik
    fi
...
verify_required_files() {
    REPO_URL="https://raw.githubusercontent.com/MarcusHoltz/Authentik-Traefik-MacVLAN/refs/heads/main"
...
echo ""
echo "Make sure have a wildcard record in your DNS pointing: "
echo "   *.${DOMAIN}   :set to:   ${TRAEFIK_MACVLAN_IP} "
echo ""
echo "You still need to configure Authentik:"
echo "http://auth.examplesetup.com/if/flow/initial-setup/"
echo ""
echo "Also if you dont configure portainer within 5min, it will need reset."
echo ""
echo "You can access your applications at:"
echo ""
echo "- Traefik Dashboard:    http://traefik.${DOMAIN}"
echo "- Authentik Login:      http://auth.${DOMAIN}"
...
echo "- Whoami1 forwardAuth:  http://whoami1.${DOMAIN}"
